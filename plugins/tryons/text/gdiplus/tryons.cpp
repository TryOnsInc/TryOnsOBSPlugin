...
// Networking includes and globals
...
static std::atomic<bool> network_thread_running{false};
...
// Definition for updating the text source (called from the network thread)
extern "C" void tryons_text_source_network_update(const char *new_text)
{
	if (!g_text_source)
		return;
	obs_data_t *settings = obs_source_get_settings(g_text_source);
	if (!settings)
		return;
	obs_data_set_string(settings, "text", new_text);
	obs_source_update(g_text_source, settings);
...
bool obs_module_load(void)
{
	...
	si.get_name = [](void *) {
		return obs_module_text("Tryons");
	};
	si.create = [](obs_data_t *settings, obs_source_t *source) {
		// Save the created text source globally
		g_text_source = source;
...
static void network_thread_func()
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
		blog(LOG_WARNING, "WSAStartup failed");
		return;
	}
#endif
	...
	if (sock < 0) {
		blog(LOG_WARNING, "Failed to create socket");
#ifdef _WIN32
		WSACleanup();
#endif
		return;
	...
	addr.sin_port = htons(12345); // Change the port as desired
	addr.sin_addr.s_addr = INADDR_ANY;

	if (bind(sock, (sockaddr *)&addr, sizeof(addr)) < 0) {
		blog(LOG_WARNING, "Bind failed");
#ifdef _WIN32
		closesocket(sock);
		WSACleanup();
#else
	...
	char buffer[1024];
	while (network_thread_running.load()) {
		int recv_len = recvfrom(sock, buffer, static_cast<int>(sizeof(buffer) - 1), 0, (sockaddr *)nullptr,
					(int *)nullptr);
		if (recv_len > 0) {
			buffer[recv_len] = '\0';
			// Call the network update function to update the text source
			tryons_text_source_network_update(buffer);
		}
	}
